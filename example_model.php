...
//        $length = $this->ci->configs_array['url_length'];//get length from $container
    ...
    /**
     * Kluge function put in place to make up for the fact that the DBAL was not designed around multiple transactions in the same session
     * 
     */
    protected function _refreshConnection()
    {
        if( ! $this->connectionObj->conn){
            $this->connectionObj->connect_2db();
        }
    }
}